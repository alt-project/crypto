...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "EnergyCoin " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. EnergyCoin is probably already running."), addrBind.ToString().c_str());
        else
...
//        printf("Error: NewThread(ThreadStakeMinter) failed\n");
    ...
    // ppcoin: mint proof-of-stake blocks in the background
    // default enabled; pos=0 set in conf to disable pos
    if (GetBoolArg("-pos", true)) {
        printf("Stake minting enabled at startup.\n");
        if (!NewThread(ThreadStakeMinter, pwalletMain))
            printf("Error: NewThread(ThreadStakeMinter) failed\n");
    } else {
        printf("Stake minting disabled at startup (pos=0).\n");
    }
